...
class Coin {
  ...
  draw() {
    ctx.drawImage(this.image, this.x - this.radius - camera.x, this.y - this.radius, this.radius * 2, this.radius * 2);
  }
}
  ...
  platforms.forEach((platform) => {
    if (!platform.coinsGenerated) {
      ...
      for (let i = 0; i < numberOfCoins; i++) {
        ...
        coins.push(new Coin(coinX, coinY, 10, 'dollars.png')); // Replace 'YOUR_IMAGE_URL' with the URL of your image
      }
...
function detectPlatformCollision() {
  ...
  function drawScoreBox() {
    drawRoundedRect(5, 5, 200, 35, 5, 'white', 'black', 3);
    ctx.fillStyle = 'black';
    ctx.font = '20px Arial';
    ctx.fillText(`Money earned: $ ${player.score}`, 10, 30);
  }
  
  function drawHighScoreBox() {
    drawRoundedRect(canvas.width - 250, 5, 250, 40, 5, 'white', 'black', 3);
    ctx.fillStyle = 'black';
    ctx.font = '20px Arial';
    ctx.fillText(`Lifetime Earnings: $ ${highScore}`, canvas.width - 220, 30);
  }
  ...
  function drawGameOverAndReset() {
    if (gameOver) {
      ctx.fillStyle = 'red';
      ctx.font = '40px Arial';
      ctx.fillText('You lost all your money to Meta!!', canvas.width / 2 - 280, canvas.height / 2);
  
      ctx.fillStyle = 'blue';
      ctx.fillRect(canvas.width / 2 - 60, canvas.height / 2 + 20, 120, 40);
      ctx.fillStyle = 'white';
      ctx.font = '25px Arial';
      ctx.fillText('Restart', canvas.width / 2 - 40, canvas.height / 2 + 48);
    }
  }
